...
# Optional env:
#   CERT_CN   signing identity Common Name (default: "Developer ID Application: PDF Tools")
#   CERT_OU   organizational unit / team    (default: "PDFTOOLS01")
#   CERT_O    organization                  (default: "PDF Tools")
#   OUT_DIR   output directory              (default: ./cert-out)
set -euo pipefail

CN="${CERT_CN:-Developer ID Application: PDF Tools}"
OU="${CERT_OU:-PDFTOOLS01}"
O="${CERT_O:-PDF Tools}"
...
# A certificate without an OU is discovered by Tauri and then rejected with
# "certificate missing organization unit for common name", several minutes into
# a release build. Fail here instead, where it costs nothing.
subject="$("$OPENSSL" x509 -in cert.pem -noout -subject)"
if ! printf '%s' "$subject" | grep -q "OU=${OU}"; then
  echo "::error::the generated certificate carries no OU=${OU}; Tauri would reject it" >&2
  exit 1
fi
